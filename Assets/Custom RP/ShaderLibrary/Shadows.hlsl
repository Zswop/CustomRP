...
ShadowData GetShadowData(float3 positonWS, float depthVS, float dither) {
	ShadowData data;
	...
	data.strength = FadedShadowStrength(
		depthVS, _ShadowDistanceFade.x, _ShadowDistanceFade.y);
	int i;
	for (i = 0; i < _CascadeCount; i++) {
		float4 sphere = _CascadeCullingSpheres[i];
		float distanceSqr = DistanceSquared(positonWS, sphere.xyz);
...
float GetCascadedShadow(DirectionalShadowData directional, ShadowData global, 
	float3 positionWS, float3 interpolatedNormalWS) {
	float3 normalBias = interpolatedNormalWS *
	...
	if (global.cascadeBlend < 1.0) {
		normalBias = interpolatedNormalWS *
...
float GetDirectionalShadowAttenuation(DirectionalShadowData directional, ShadowData global, 
	...
	if (directional.strength * global.strength <= 0.0) {
		...
		shadow = GetCascadedShadow(directional, global, positionWS, interpolatedNormalWS);
...
float GetOtherShadow(OtherShadowData other, ShadowData global, 
	float3 positionWS, float3 interpolatedNormalWS) {
	...
	float3 surfaceToLight = other.lightPositionWS - positionWS;
	float distanceToLightPlane = dot(surfaceToLight, lightPlane);
	float3 normalBias = interpolatedNormalWS * (distanceToLightPlane * tileData.w);
	float4 positionSTS = mul(_OtherShadowMatrices[tileIndex], float4(positionWS + normalBias, 1.0));
	return FilterOtherShadow(positionSTS.xyz / positionSTS.w, tileData.xyz);
}

float GetOtherShadowAttenuation(OtherShadowData other, ShadowData global, 
	float3 positionWS, float3 interpolatedNormalWS) {
#if !defined(_RECEIVE_SHADOWS)
	return 1.0;
#endif
	...
	if (other.strength * global.strength <= 0.0) {
		...
		shadow = GetOtherShadow(other, global, positionWS, interpolatedNormalWS);
		shadow = MixBakedAndRealtimeShadows(
